...
int main(int argc, char* args[]){
    ...
    // PONTEIRO QUE CARREGA O ICONE DA JANELA
    SDL_Surface *icone;

    // FORMATO DE AÚDIO
    Uint16 formato = AUDIO_S16SYS;

    // DEFININDO SE O AUDIO É MONO OU STERÉO DENTRE OUTRAS COISAS
    Mix_OpenAudio(22050, formato, 2, 4096);

    // PONTEIRO QUE IRÁ CARREGAR A MÚSICA
    Mix_Music *musica;

    //CARREGANDO A MÚSICA DE FUNDO DO JOGO
    musica = Mix_LoadMUS("fundo.mp3");

    // CARREGANDO O ICONE DA JANELA
    icone = IMG_Load("icon.png");

    // DESABILITAR A TRANSPARÊNCIA
    glDisable(GL_BLEND);
    ...
    SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 16); // PARA TRABALHAR COM 3D / O QUE NÃO VAI SER O CASO
    SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);

    // TAMANHO DA JANELA - 600 EM 'X' 400 EM 'Y' E 32 BITS POR PIXEL, TIPO DE JANELA OPEN GL
    SDL_SetVideoMode(600, 400, 32, SDL_OPENGL);

    // AQUI ESTAMOS PUXANDO O PONTEIRO ICONE PARA FICAR NO CANTO SUPERIOR ESQUERDO DA JANELA / NULL É O CAMINHO DA IMAGEM QUE NO CASO JÁ FOI CARREGADA
    ...
    // ÁREA EXIBIDA
    glViewport(0, 0, 600, 400); // 600 EM X E 400 EM Y
    ...
    while(executando)
    {
        ...
        // ANIMAÇÃO
        SDL_GL_SwapBuffers(); // PARA TRABALHAR COM ANIMAÇÕES - NO CASO ESTÁ DESABILITADA
